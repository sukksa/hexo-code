...
- `slot render: (h)=>{h(App)}`, //DOM 成员（3/3） 
...
- `watch` //数据成员（3/6） 
	...
	Vue的监视属性，整个为一个对象，键是需要观察的表达式，值是对应回调函数 
...
- `functional` //杂项（6/6） 
	使组件无状态（没有 `data` ）和无实例（没有 `this` 上下
...
**注意:**这个方法返回实例自身,因而可以链式调用其它实例方法
        ...
        var MyComponent = Vue.extend({ template: '<div>Hello!</div>' }) // 创建并挂载到 #app (会替换 #app)
